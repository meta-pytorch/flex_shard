...
from flex_shard.dist_muon.optimizer_reshard import (
    BlockShard,
    BucketConfig,
    ComputeLayout,
    Owned,
)
from flex_shard.dist_muon.optimizer import (
...
def _checkpoint_model(optimizer):
    """Expose the optimizer's real parameters under their FQNs for PyTorch DCP."""
    ...
    for group in optimizer.param_groups:
        for name, parameter in zip(group["param_names"], group["params"], strict=True):
            ...
            *path, leaf = name.split(".")
            ...
            module.register_parameter(leaf, parameter)
    return model
...
class TestDistMuon(DTensorTestBase):
    ...
    def test_matches_plain_muon_across_flat_checkpoint(self):
        ...
        flat_state_dict = get_optimizer_state_dict(
            _checkpoint_model(optimizer),
            optimizer,
        ...
        set_optimizer_state_dict(
            _checkpoint_model(resumed_optimizer),
            resumed_optimizer,
            flat_state_dict,
            options=StateDictOptions(flatten_optimizer_state_dict=True),
        )
