...
class _RafSavedTensorContext:
    ...
    def __init__(self) -> None:
        self.tensor_handles = WeakTensorKeyDictionary()

    def register(self, tensor: torch.Tensor, handle: Any) -> None:
        self.tensor_handles[tensor] = handle
        ...
        if base_handle is not None:
            self.tensor_handles[base] = base_handle

    def pack(self, tensor: torch.Tensor) -> Any:
        handle = self.tensor_handles.get(tensor)
        if handle is not None:
            return handle

        base = getattr(tensor, "_base", None)
        if base is not None:
            base_handle = self.tensor_handles.get(base)
            if base_handle is not None:
...
_ACTIVE_RAF_SAVED_TENSOR_CONTEXT: ContextVar[_RafSavedTensorContext | None] = (
